...
p = inputParser;
addRequired(p, 'ks_folders', @(x) isstring(x) && isvector(x) && length(x) > 1);
...
addParameter(p, 'metrics_name', 'metrics.csv', @(x) mustBeFileType(x, {".csv", ".tsv"}));
...
% if it is a list of extensions to check
if iscell(ext)
    for i = 1:length(ext)
    ...
    error("The provided name ('%s') must end with ('%s')", filename, strjoin(ext, ", "))
else
