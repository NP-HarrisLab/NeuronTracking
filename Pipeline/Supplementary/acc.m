...
% recovery rate
[~,same_idx1,same_idx2] = intersect(truth(:,2), all_match(:,3),'stable');
label_matched = truth(same_idx1,:);
all_matched = all_match(same_idx2,:);
all_matched = all_match(same_idx2,2:3);
[~, all_idx] = ismember(label_matched, all_matched, 'rows');
eval(1) = nnz(all_idx)/length(all_idx);

% accuracy
[~,same_idx1_th,same_idx2_th] = intersect(truth(:,2), threshold_match(:,3),'stable');
