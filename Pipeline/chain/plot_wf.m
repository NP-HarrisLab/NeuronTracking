...
wave1 = readNPY(fullfile(data_path1, all_input{id}.input.wf_name));
wave2 = readNPY(fullfile(data_path2, all_input{id}.input.wf_name));
...
rows = repelem(startRow:endRow, numCol_to_plot);
siteCount = 0;
plotSite = cell(1, numSite);

% Loop through each site to find relevant site waveforms
for isite = 1:numSite 
    expected_y = rows(isite) * vSep + min(yC);
    this_site = find(xC == x_site(isite) & yC == expected_y);

    if ~isempty(this_site)
        siteCount = siteCount + 1;
        site(siteCount) = this_site;

        plotSite{id}(siteCount, :) = peakWf1(site(siteCount), :);
...
rows = repelem(startRow:endRow, 2);
siteCount = 0;
for isite = 1:numSite %find relevant site index
    ...
    if ~isempty(this_site)
        ...
        plotSite{id+1}(siteCount,:) = peakWf2(site(siteCount), :);
